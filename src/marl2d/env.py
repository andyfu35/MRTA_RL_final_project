...
def _rects_too_close(a: np.ndarray, b: np.ndarray, spacing: float) -> bool:
    ...
    return abs(ax - bx) < (aw + bw) * 0.5 + spacing and abs(ay - by) < (ah + bh) * 0.5 + spacing


def _ray_aabb_distance(x: float, y: float, dx: float, dy: float, rect: np.ndarray) -> float | None:
    cx, cy, width, height = (float(v) for v in rect)
    bounds = ((cx - width * 0.5, cx + width * 0.5), (cy - height * 0.5, cy + height * 0.5))
    t_near = 0.0
    t_far = float("inf")
    for origin, direction, (low, high) in ((x, dx, bounds[0]), (y, dy, bounds[1])):
...
class VectorArena2D:
    ...
    State: [num_envs, 4, (x, y, theta)]
    Action: [num_envs, 4, (left, right)] in [-1, 1]
    Obstacles: [num_envs, count, (center_x, center_y, width, height)]
    """

    observation_dim = 21
    action_dim = 2

    def __init__(self, num_envs: int, env_cfg: dict[str, Any], reward_cfg: dict[str, Any], seed: int = 0) -> None:
        if num_envs <= 0:
    ...
    def _generate_obstacles_for_env(self, env_index: int) -> np.ndarray:
        ...
        while len(placed) < self.obstacle_count and attempts < max_attempts:
            attempts += 1
            width = float(self.rng.uniform(float(cfg["min_width"]), float(cfg["max_width"])))
            height = float(self.rng.uniform(float(cfg["min_height"]), float(cfg["max_height"])))
            margin = float(cfg.get("border_margin", 0.0))
            x_low, x_high = margin + width * 0.5, self.width - margin - width * 0.5
            y_low, y_high = margin + height * 0.5, self.height - margin - height * 0.5
            if x_low >= x_high or y_low >= y_high:
                raise ValueError("Obstacle size and border margin leave no valid placement area")
            rect = np.array([self.rng.uniform(x_low, x_high), self.rng.uniform(y_low, y_high), width, height], dtype=np.float32)
            if any(_point_to_rect_distance(float(p[0]), float(p[1]), rect) < float(cfg.get("spawn_clearance", 0.0)) for p in spawn_points):
                continue
            if _point_to_rect_distance(float(self.goal[0]), float(self.goal[1]), rect) < float(cfg.get("goal_clearance", 0.0)):
                continue
            if any(_rects_too_close(rect, old, float(cfg.get("min_spacing", 0.0))) for old in placed):
                continue
            placed.append(rect)
    ...
    def _regenerate_obstacles(self, indices: np.ndarray) -> None:
        for env_index in np.asarray(indices, dtype=np.int64):
            self.obstacles[env_index] = self._generate_obstacles_for_env(int(env_index))
    ...
    def reset(self, seed: int | None = None) -> np.ndarray:
        ...
        if self.reset_jitter > 0:
            self.state[:, :, :2] += self.rng.uniform(-self.reset_jitter, self.reset_jitter, size=(self.num_envs, 4, 2)).astype(np.float32)
    ...
    def reset_indices(self, mask: np.ndarray) -> None:
        ...
        if self.reset_jitter > 0:
            self.state[mask, :, :2] += self.rng.uniform(-self.reset_jitter, self.reset_jitter, size=(count, 4, 2)).astype(np.float32)
    ...
    def _goal_distances(self, state: np.ndarray | None = None) -> np.ndarray:
        ...
        return np.linalg.norm(s[:, :2, :2] - self.goal[None, None, :], axis=-1)
    ...
    def _integrate(self, actions: np.ndarray) -> np.ndarray:
        ...
        old = self.state
        theta = old[:, :, 2]
        new_theta = _wrap_angle(theta + yaw_rate * self.dt)
        proposed = old.copy()
        ...
        proposed[:, :, 0][turning] = old[:, :, 0][turning] + radius[turning] * (np.sin(new_theta[turning]) - np.sin(theta[turning]))
        proposed[:, :, 1][turning] = old[:, :, 1][turning] - radius[turning] * (np.cos(new_theta[turning]) - np.cos(theta[turning]))
        proposed[:, :, 2] = new_theta
        return proposed

    def _collision_mask(self, proposed: np.ndarray) -> np.ndarray:
        collided = np.zeros((self.num_envs, 4), dtype=bool)
        x, y, radius = proposed[:, :, 0], proposed[:, :, 1], self.robot_radius
        collided |= (x < radius) | (x > self.width - radius) | (y < radius) | (y > self.height - radius)
        for e in range(self.num_envs):
            for rect in self.obstacles[e]:
                cx, cy, width, height = (float(v) for v in rect)
                nx = np.clip(x[e], cx - width * 0.5, cx + width * 0.5)
                ny = np.clip(y[e], cy - height * 0.5, cy + height * 0.5)
                collided[e] |= (x[e] - nx) ** 2 + (y[e] - ny) ** 2 < radius**2
        min_robot_dist2 = (2.0 * radius) ** 2
        for a in range(4):
            for b in range(a + 1, 4):
                hit = np.sum((proposed[:, a, :2] - proposed[:, b, :2]) ** 2, axis=-1) < min_robot_dist2
                collided[:, a] |= hit
                collided[:, b] |= hit
        return collided
    ...
    def _blocker_proximity(self) -> np.ndarray:
        ...
        dist = np.linalg.norm(blockers[:, :, None, :] - runners[:, None, :, :], axis=-1).min(axis=-1)
        return np.clip(1.0 - dist / max(self.width, self.height), 0.0, 1.0)
    ...
    def step(self, actions: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict[str, np.ndarray]]:
        ...
        rr, rb = self.reward_cfg["runner"], self.reward_cfg["blocker"]
    ...
    def _ray_distance(self, env_index: int, x: float, y: float, angle: float) -> float:
        ...
        if abs(dx) > 1e-9:
            for wall_x in (0.0, self.width):
                t = (wall_x - x) / dx
                if t > 0 and 0.0 <= y + t * dy <= self.height:
                    best = min(best, t)
        if abs(dy) > 1e-9:
            for wall_y in (0.0, self.height):
                t = (wall_y - y) / dy
                if t > 0 and 0.0 <= x + t * dx <= self.width:
                    best = min(best, t)
        ...
        for e in range(self.num_envs):
            for i in range(4):
                ...
                for j in range(4):
                    if j == i:
                        continue
                    bx, by = _body_frame(float(self.state[e, j, 0] - x), float(self.state[e, j, 1] - y), theta)
